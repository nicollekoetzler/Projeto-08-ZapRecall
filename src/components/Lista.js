...
export default function Lista({setIconBottom, iconBottom}){
...
const itemsQuestions = [
    ...
    return (
        <div className="lista">
            <ul>
                {itemsQuestions.map((item, index) => <Questions key={index} setIconBottom={setIconBottom} iconBottom={iconBottom} question={item.question} shownquestion={item.shownquestion} answer={item.answer} />)}
            </ul>
        </div>
    )
}